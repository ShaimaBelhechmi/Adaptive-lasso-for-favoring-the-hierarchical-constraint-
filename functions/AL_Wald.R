...
## Function for calculating the wald test 
  fWald <- function(i) {
    Wtest <- coxph(Surv(time, status) ~ XT[, i], data = data)$wald.test
    return(Wtest)
  }
  
  weights <- 1 / as.numeric(sapply(c(1:nbiom), fWald))
  weights <- c(rep(weights,each=2), 0)
